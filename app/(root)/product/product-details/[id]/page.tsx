      ...
      <div className="col-span-4">
        ...
        <div className="mt-2 flex items-center space-x-2">
          <div className="flex items-center">
            {starArray.map((star) => {
              return (
                <StarIcon key={Math.random()*1000} size={20}
                fill='yellow' className='text-yellow-600' />
              )
            })}
          </div>
          <p className='text-base text-gray-700 font-semibold'>
            ({singleProduct?.rating.count} Reviews)</p>
        </div>
        {/* Line */}
        ...
        <p className='mt-4 text-sm text-black opacity-70 font-semibold'>Category : {singleProduct.category}</p>
        <p className='mt-2 text-sm text-black opacity-70 font-semibold'>Tag : Shop Now</p>
        <p className='mt-2 text-sm text-black opacity-70 font-semibold'>SKU : {Math.random() * 500}</p>
        {/* Add To Cart Button */}
